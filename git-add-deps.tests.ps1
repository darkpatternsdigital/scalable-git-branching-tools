Describe 'git-add-deps' {
    ...
    Context 'without a remote' {
        ...
        It 'halts if no branch was provided and none is checked out' {
            ...
            { & ./git-add-deps.ps1 -dependency 'feature/FOO-76' } | Should -Throw
        ...
        It 'works on the current branch' {
            $mocks = @(
                ...
                Initialize-DependencyBranches @{ 'rc/2022-07-14' = @("feature/FOO-123","feature/XYZ-1-services") }
                ...
                Initialize-LocalActionMergeBranchesSuccess `
                    -dependencyBranches @('feature/FOO-76') -resultCommitish 'result-commitish' `
            ...
            & ./git-add-deps.ps1 'feature/FOO-76'
            $fw.assertDiagnosticOutput | Should -BeNullOrEmpty
            Invoke-VerifyMock $mocks -Times 1
        }

        It 'works if there are no dependency branches' {
            $mocks = @(
                Initialize-LocalActionAssertExistence -branches @('rc/2022-07-14', 'feature/FOO-76') -shouldExist $true
                Initialize-CurrentBranch 'rc/2022-07-14'
                Initialize-AssertValidBranchName 'rc/2022-07-14'
        ...
        It 'works locally with multiple branches' {
            $mocks = @(
                ...
                Initialize-LocalActionMergeBranchesSuccess `
                    -dependencyBranches @('feature/FOO-76', 'feature/FOO-84') -resultCommitish 'result-commitish' `
                    -source 'rc/2022-07-14' `
                    -mergeMessageTemplate "Merge '{}' to rc/2022-07-14"
                Initialize-FinalizeActionSetBranches @{
                    '$dependencies' = 'new-commit'
            ...
            & ./git-add-deps.ps1 'feature/FOO-76','feature/FOO-84' -m ""
            $fw.assertDiagnosticOutput | Should -BeNullOrEmpty
            Invoke-VerifyMock $mocks -Times 1
        }
        ...
        It 'works locally against a target branch' {
            ...
            & ./git-add-deps.ps1 -dependency 'feature/FOO-76' -target 'rc/2022-07-14' -m ""
            $fw.assertDiagnosticOutput | Should -BeNullOrEmpty
            Invoke-VerifyMock $mocks -Times 1
        }
        ...
        It 'works locally with multiple branches against a target branch' {
            ...
            & ./git-add-deps.ps1 'feature/FOO-76','feature/FOO-84' -target 'rc/2022-07-14' -m ""
            $fw.assertDiagnosticOutput | Should -BeNullOrEmpty
            Invoke-VerifyMock $mocks -Times 1
        }
    }
    ...
    Context 'with a remote' {
        ...
        It 'works on the current branch' {
            ...
            & ./git-add-deps.ps1 @('feature/FOO-76') -m ""
            $fw.assertDiagnosticOutput | Should -BeNullOrEmpty
            Invoke-VerifyMock $mocks -Times 1
        }
        ...
        It 'does nothing if the added branch is already included' {
            $mocks = @(
                ...
                Initialize-LocalActionAssertPushedSuccess 'rc/2022-07-14'
                Initialize-DependencyBranches @{ 'rc/2022-07-14' = @("feature/FOO-123","feature/XYZ-1-services","infra/shared") }
                Initialize-LocalActionSimplifyDependencyBranchesSuccess `
            ...
            { & ./git-add-deps.ps1 @('infra/shared') -target 'rc/2022-07-14' } | Should -Throw
        ...
        It 'simplifies if the added branch makes another redundant' {
            $mocks = @(
                ...
                Initialize-LocalActionMergeBranchesSuccess `
                    -dependencyBranches @('feature/FOO-123') -resultCommitish 'result-commitish' `
                    -source 'rc/2022-07-14' `
                    -mergeMessageTemplate "Merge '{}' to rc/2022-07-14"
                Initialize-FinalizeActionSetBranches @{
                    '$dependencies' = 'new-commit'
            ...
            & ./git-add-deps.ps1 @('feature/FOO-123') -target 'rc/2022-07-14' -m ""
        ...
        It 'handles when the target branch doesn''t exist locally' {
            ...
            & ./git-add-deps.ps1 @('feature/FOO-76') -target 'rc/2022-07-14' -m ""
            $fw.assertDiagnosticOutput | Should -BeNullOrEmpty
            Invoke-VerifyMock $mocks -Times 1
        }
        ...
        It 'outputs a helpful message if it fails' {
            $mocks = @(
                ...
                Initialize-LocalActionMergeBranchesFailure `
                    -dependencyBranches @('feature/FOO-76') `
        ...
        It 'ensures the remote is up-to-date' {
            ...
            { & ./git-add-deps.ps1 @('feature/FOO-76') -target 'rc/2022-07-14' -m "" } | Should -Throw
